...
fn main() {
    ...
    match mode.as_str() {
        "vertex" => {
            pagerank(&NodesEdgesMemMapper::new(&name), nodes, 0.85f32, 20)
        },
        "hilbert" => {
            COST::bench::bench(|iters| {
                pagerank(&UpperLowerMemMapper::new(&name), nodes, 0.85f32, iters)
            })
        },
        "compressed" => {
            pagerank(&DeltaCompressedReaderMapper::new(|| BufReader::new(File::open(&name).unwrap())), nodes, 0.85f32, 20)
        },
        _ => { println!("unrecognized mode: {:?}", mode); },
    }
}

fn pagerank<G: EdgeMapper>(graph: &G, nodes: u32, alpha: f32, iters: usize) {
    ...
    for _iteration in 0 .. iters {
        ...
        // UNSAFE:
        graph.map_edges(|x, y| { unsafe { *dst.get_unchecked_mut(y as usize)
            += *src.get_unchecked(x as usize); }});
    }
}
